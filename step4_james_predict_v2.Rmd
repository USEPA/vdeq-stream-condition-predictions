...
# current ssn
j_ssn1a <- SSN2::ssn_import("E:/R_vdeq_sci/Working/Data/neptune_analysis/ssn_objects/James_071024.ssn", predpts = c("Preds_2019_2020", "Preds_2021_2022"),
overwrite = FALSE)
...
Note that both obs and preds need to be in the SSN object that is modeled. This results in the SSN model object having all the variables needed for the SSN predict function.
	...
	3. Log total nitrogen. Do not square vsci as the response variable is not included in the preds data frame.
...
names(DFobsz)
DFobsz2 <- dplyr::select(DFobsz, c(st_id_tren, pct_for_w_emplog:imp_w_pres))
...
st_write(ssn_wswq_fit_reml1, dsn = file.path(getwd(), "ssn_wswq_fit_reml1.gpkg"), layer = "ssn_wswq_fit_reml1", driver = "GPKG", quiet = FALSE)
...
st_write(z4rssn_wswq_reml1, dsn = file.path(getwd(), "z4rssn_wswq_reml1.gpkg"), layer = "z4rssn_wswq_reml1", driver = "GPKG", quiet = FALSE)
...
write.csv(models_ml_ssn, file="E:/R_vdeq_sci/Working/Data/neptune_analysis/scripts_by_basin/tables_sfs/four_models_ml_ssn.csv",sep=",",row.names=F)
```
...
ggsave("E:/R_vdeq_sci/Working/Data/neptune_analysis/scripts_by_basin/figures_sfs/ssn_4models_dwplot_v1_20240430.png", width = 12, height = 8, units = "in")
...
ggsave("E:/R_vdeq_sci/Working/Data/neptune_analysis/scripts_by_basin/figures_sfs/ssn_4models_dwplot_v1_20240501.png", width = 10, height = 8, units = "in")
```
...
summary(j_ssn3$preds$Preds_2019_2020$pct_imp_w_emplog)
summary(j_ssn3$preds$Preds_2019_2020$elev_ws)
summary(j_ssn3$preds$Preds_2019_2020$do)
summary(j_ssn3$Preds_2019_2020$vahusb_JL)
...
ggplot(predict1ws_vsci, aes(x = vsci_pred, y = vsci)) + geom_point() + geom_abline()

preds2021_2022_v1 <- ggplot(predict1ws_vsci, aes(x = vsci_pred, y = vsci, label = st_id_tren)) + geom_point() + geom_abline() + geom_text(aes(label =ifelse(vsci<40, as.character(st_id_tren), '')), hjust = -0.1, vjust = 0)+ labs(x = "Predicted VSCI", y = "Observed VSCI, 2021-2022", title = "Watershed SSN") + xlim(35,80) + ylim(35,80)

png(file="figures_sfs/preds2021_2022_v1.png",width=6,height=3,units="in",res=150)
  preds2021_2022_v1
dev.off()

cor(predict1ws_vsci$vsci_pred,predict1ws_vsci$vsci, method = "pearson")
...
# added error bars
ggplot(predict1ws_vsci, aes(x = vsci_pred, y = vsci)) + geom_point() + geom_abline() + geom_errorbar(aes(xmax = vsci_upr, xmin = vsci_lwr)) + labs(x = "Predicted VSCI", y = "Observed VSCI, 2019-2020")

mapview(predict1ws_vsci)
...
st_write(preds_cov_1920, dsn = file.path(getwd(), "preds_cov_1920.gpkg"), layer = "preds_cov_1920", driver = "GPKG", quiet = FALSE)
```
...
mapview(preds_bind1, zcol = "stdres", cex = "stdres", alpha.regions = .8, legend = TRUE, popup = popupTable(preds_bind1, zcol = c("stdres"))) + mapview(stream)
