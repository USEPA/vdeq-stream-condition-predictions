...
## 3.3.1 SFS Meeting Trend Stations
...
## 3.40 Onsite, Watershed and Catchment Area Covariates
...
zlogit <- dplyr::select(DFobsz, c(station_id_2, vsci,pct_imp_w, pct_imp_w_emplog,pct_imp_rp_w, pct_imp_rp_w_emplog, pct_imp_c, pct_imp_c_emplog, pct_imp_rp_c, pct_imp_rp_c_emplog))
...
ggplot(data = zlogit,
  ...
  geom_point()
# car::logit transform
zlogit$car_logit = car::logit(p = zlogit$pct_imp_w,
                              adjust = .00025)

#car::logit transform
ggplot(data = zlogit,
       aes(x =car_logit, y = vsci))+
  geom_point()


######## alternate discussion for pct_w
zlogit2 <- dplyr::select(DFobsz, c(station_id_2, vsci,pct_for_w,pct_for_w_emplog))
...
ggplot(data = zlogit2,
  ...
  geom_point()
# car::logit transform
zlogit2$car_logit = car::logit(p = zlogit2$pct_for_w,
                              adjust = .025)
