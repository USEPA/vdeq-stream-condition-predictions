...
We now turn our attention to the covariates, or predictor variables, we want to use to model and predict VSCI. The SSN object we are using was built using the STARS Toolbox in ArcGIS.
...
Steps are 1) select station_id from DFobs with geometry, 3) use unique.data.frame to get unique stations, and then 4) use st_nn. Like the five-number distance summary done in script 1, this summarization of distance to a stations first closest neighbor is done to get a getting understanding of Euclidean distances.
```{r dist_knn1}
...
head(dists_knn1)
summary(dists_knn1) # with units in meters.
...
The StreamCat covariates have a COMID/FeatureID associated with them, with those identifiers coming from the reaches in NHDPlusV2. NHDPlusv2 was the originating dataset for the National Stream Internet (NSI) flowlines or edges.
...
# ---EDA code commented out---- Comment back in to see additional EDA options that were explored ### 
...
Based on Schmid et al. 2011 and Hill et al. 2017, consider mean percent sand content of soils in a watershed (SandWs) as a possible covariate.
...
Ellen D'Amico calculated waterbody variables. Ellen D'Amico is a GIS analyst, originally with Pegasus Technical Services, Inc. and now with GDIT as part of the EPA National Geospatial Support Team (NGST).
...
## 3.25 PRISM data
The PRISM data was obtained from  <https://prism.oregonstate.edu/recent/>.
...
This code came from Travis Linscome-Hatfield, statistician at Neptune. Applying empirical logit only to obs.
...
## 4.20 Logit Example of Impervious Cover
