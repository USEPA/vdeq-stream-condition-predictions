...
11/25/2024 using James_071024_pluspreds as ssn object as has all 46 prediction points in one shapefile. And bringing additional covariates. Needing to assign 0's to preds_2019_2022 that initially had dist_nearest_up_wb_km set to NA for 31 of 48 prediction points.

Keep logging going below and will delete when get final version of script.
...
On 12/16/2025 added binomial wbc, wbc_bin, coded as numeric 0 or 1 similar to Dumelle et al. 2023 coding in National Lake assessment conductivity paper.
...
z4rssn_wswq_reml1 <- dplyr::select(z4rssn_wswq_reml1, c(station_id, station_id_2, year, vahusb, do, tn, tothab, l_tn, vscivcpmi, pct_imp_c, pct_imp_w, elev_ws, pct_imp_w_emplog, vsci, .fitted, .std.resid))

saveRDS(z4rssn_wswq_reml1, file = "outputs/z4rssn_wswq_reml1.rds")
...
On 12/16/2024 first using a4 reml model and will compare to z5 reml model with distance covariate as tht provides a better fit based on smaller AICc value.
```{r fitted_interval_plot}
...
aug_z4ssn_wswq_reml1 <- augment(z4ssn_wswq_reml1, drop = FALSE)
class(aug_z4ssn_wswq_reml1)
names(aug_z4ssn_wswq_reml1)

head(aug_z4ssn_wswq_reml1$.fitted)
...
# plot
plot(tx,ty,xlim=range(na.omit(tx)),ylim=range(na.omit(ty),na.omit(op95)),xlab="VSCI Predicted Value",ylab="VSCI Observed Value",main="Spatial Model Observed vs. Predicted z4 reml")
abline(0,1,col="red")
...
On 12/16/2024 in near future will likely comment out this code chunk as no support found for including these additional covariates.
...
This follows coding as used in Dumelle et al. 2023 paper. Using that coding produced a poorer fitting more complicated model than z4. Using just the continuous covariate of distance, as done on 11/19/2024, produces a better fitting model. What does added variable plot of that model look like?
...
a5predict1ws_vsci <- augment(a5ssn_wswq_reml1, newdata = "Preds_2019_2022", se_fit = TRUE, interval = c("prediction"))
# class(predict1ws_vsci)
# names(predict1ws_vsci)
a5predict1ws_vsci$vsci_pred <- round(a5predict1ws_vsci$.fitted,1)
a5predict1ws_vsci$vsci_lwr <- round(a5predict1ws_vsci$.lower,1)
a5predict1ws_vsci$vsci_upr <- round(a5predict1ws_vsci$.upper,1)
...
ggplot(a5predict1ws_vsci, aes(x = vsci_pred, y = vsci)) + geom_point() + geom_abline()

a5preds2019_2022_v1 <- ggplot(a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, label = station_id_2)) + geom_point() + geom_abline() + geom_text(aes(label =ifelse(vsci<45, as.character(station_id_2), '')), hjust = -0.1, vjust = 0)+ labs(x = "Predicted VSCI", y = "Observed VSCI, 2019-2022", title = "a5reml Watershed SSN JU Partitioned") + xlim(35,80) + ylim(35,80)

png(file="figures_sfs/a5preds2019_2022_v1.png",width=6,height=3,units="in",res=150)
  a5preds2019_2022_v1
dev.off()

cor(a5predict1ws_vsci$vsci_pred,a5predict1ws_vsci$vsci, method = "pearson")

#2019-2022 a5reml correlation with partition factor of ju and distance covariate is 0.57

# 2019-2022 correlation w/partition factor of ju and additional covariates was 0.54
as.data.frame(a5predict1ws_vsci)|>
...
ggplot(a5predict1ws_vsci, aes(x = vsci_pred, y = vsci)) + geom_point() + geom_abline() + geom_errorbar(aes(xmax = vsci_upr, xmin = vsci_lwr)) + labs(x = "Predicted VSCI", y = "Observed VSCI, 2019-2022")

mapview(a5predict1ws_vsci)
...
I believe this coding is fitting to 199 obs because using a5ssn_wswq_reml1. I think it should be using a5 predict1ws_vsci as it has 48 predicted sites from 2019-2022.
```{r fitted_interval_preds_plot}

# get spatial residuals from model
spatResids_preds<-residuals(a5ssn_wswq_reml1, cross.validation=T)

# pull sd of residuals and fitted values
resid_SD = sd(spatResids_preds)
fitted_vals = a5predict1ws_vsci$.fitted
...
# make sure I've got the right obj
DFpreds<-ssn_get_data(j_ssn3,name = "Preds_2019_2022")
# observed and median of our prediction intervals
ty<-DFpreds$vsci
tx<-spat.Pred.median
...
# plot
plot(tx,ty,xlim=range(na.omit(tx)),ylim=range(na.omit(ty),na.omit(op95)),xlab="VSCI Predicted Value 2019-2022",ylab="VSCI Observed Value 2019-2022",main="Spatial Model Observed vs. Predicted a5 reml")
abline(0,1,col="red")
lines(lowess(na.omit(tx),na.omit(ty)),col='blue')
...
# Watershed Predictions 2019-22
Commented out as a5 distance model had better predictive performance than the z4 partitioned model.
```{r predictions}
# check stats on pred covariates to make sure corrects preds ssn data frame being used
summary(j_ssn3$preds$Preds_2019_2022$pct_imp_w_emplog)
summary(j_ssn3$preds$Preds_2019_2022$elev_ws)
summary(j_ssn3$preds$Preds_2019_2022$do)
summary(j_ssn3$Preds_2019_2022$vahusb_JL)

summary(j_ssn3)
str(j_ssn3$obs$pct_imp_w_emplog)

# augment returns an sf object
z4predict1ws_vsci <- augment(z4ssn_wswq_reml1, newdata = "Preds_2019_2022", se_fit = TRUE, interval = c("prediction"))
...
preds2019_2022_v1 <- ggplot(z4predict1ws_vsci, aes(x = vsci_pred, y = vsci, label = st_id_tren)) + geom_point() + geom_abline() + geom_text(aes(label =ifelse(vsci<40, as.character(st_id_tren), '')), hjust = -0.1, vjust = 0)+ labs(x = "Predicted VSCI", y = "Observed VSCI, 2019-2022", title = "Watershed SSN z4 partitioned") + xlim(35,80) + ylim(35,80)

png(file="figures_sfs/preds2091_2022_v1.png",width=6,height=3,units="in",res=150)
  preds2019_2022_v1
dev.off()
...
z4preds_cov_19_22 <- dplyr::select(z4predict1ws_vsci, c(station_id, station_id_2, year, vahusb, do, tn, tothab, l_tn, vscivcpmi, pct_imp_c, elev_ws, pct_imp_w_emplog, vsci, vsci_pred, .se.fit, vsci_lwr,vsci_upr))

saveRDS(z4preds_cov_19_22, file = "z4preds_cov_19_22.rds")
