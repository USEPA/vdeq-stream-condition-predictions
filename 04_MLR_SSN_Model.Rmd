...
## 1.30 Factors:  Ecoregion,  Bioregion, VAHUSB, Year, Status and Trend Stations
...
Need to bring in HUC8 polygons. VSCICPMI scores quite high in Blue Ridge. Using vahusb as those 5 polygons are comprised of huc8s.
```{r esda_polygons}
...
# have to hard code type to get all 3 distances otherwise only
# get flowcon and flowuncon if drop type argument
ztg2 <- SSN2::Torgegram(tothab ~ 1, j_ssn3, type = c("flowcon", "flowuncon", "euclid"))
names(ztg2)
summary(ztg2$flowcon)
summary(ztg2$flowuncon)
summary(ztg2$euclid)
plot(ztg2)
plot(ztg2, type = "flowcon", main="TotHab")
plot(ztg2, type = "flowuncon", main = "TotHab")
plot(ztg2, type = "euclid", main = "TotHab")
class(ztg2)
# plot(ztg2, separate = TRUE, main = "TotHab")
...
ytg <- SSN2::Torgegram(l_tn ~ 1, j_ssn3, type = c("flowcon", "flowuncon", "euclid"))
plot(ytg, main = "Log(TN)")

ztg3 <- Torgegram(vsci ~ 1, j_ssn3, type = c("flowcon", "flowuncon", "euclid"))
# , type = c("flowcon", "flowuncon", "euclid"))
plot(ztg3, main = "VSCI")
# plot now working getting all 3 distances
# plot(ztg3, separate = TRUE, main = "VSCI")
plot(ztg3, main ="VSCI")
plot(ztg3, type = "flowcon", main="VSCI")
plot(ztg3, type = "flowuncon", main = "VSCI")
plot(ztg3, type = "euclid", main = "VSCI")

ztg4 <- Torgegram(l_tn ~ 1, j_ssn3, type = c("flowcon", "flowuncon", "euclid"))
# , type = c("flowcon", "flowuncon", "euclid"))
# plot working Torgegram object has all 3 distances
plot(ztg4, main = "Log(TN)")
# plot(ztg4, separate = TRUE, main = "Log(TN)")

print(ztg4$flowcon)
print(ztg4$flowuncon)
print(ztg4$euclid) # get output just like other distances
...
This is code I want Neptune to evaluate, chunks 21-24.
Simulate VSCI data for SSN to work out code on simulated response variables. Have sim1 simulated parameters based from y-intercept SSN model, ssn_mod_y. Have sim2 be  strong flow unconnected spatial structure and sim3 based on strong Euclidean spatial structure. Use j_ssn3 as that has variables in lowercase. Putting the simulated data into j_ssn4. My use of the term function refers to SSN tailup, taildown, or Euclidean, and shape refers distribution of residuals in the semivariogram/Torgegram as Gaussian, exponential, linear sill, etc.
...
z1 <- ssn_simulate("gaussian", j_ssn3, "obs", tailup, taildown, euclid, nugget, additive = "afv_area", mean = 65)
...
In 2.10 I simulated data based on the three-way (tail up, tail down, and Euclidean) exponential y-intercept model. Below I am taking that simulated data and testing it against different spatial autocovariance models, including a nugget or nonspatial model.
```{r evaluate_simulated_data}
...
z4 <- ssn_simulate("gaussian", j_ssn3, "obs", tailup, taildown, euclid, nugget, additive = "afv_area", mean = 65)
...
ztg4 <- SSN2::Torgegram(vscivcpmi ~ 1, j_ssn4, type = c("flowcon", "flowuncon", "euclid"))
names(ztg4)
...
z5 <- ssn_simulate("gaussian", j_ssn3, "obs", tailup, taildown, euclid, nugget, additive = "afv_area", mean = 65)
...
#double checking our base cases for variables we're selectingfor the bestglm
df_dum_vahusb$base1
df_dum_vahusb$base2
df_dum_tp$base1
df_dum_tp$base2
