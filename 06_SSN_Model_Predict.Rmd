...
zribbon_pi2 <- ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, shape = st_type, color = st_type)) +
  geom_point(aes(size = .se.fit)) + 
  geom_abline() + 
  geom_ribbon(aes(ymin = vsci_lwr, ymax = vsci_upr), fill = "grey", alpha = .4) +
  geom_line(aes( y = vsci_pred)) + 
  labs(x = "Predicted VSCI", y = "True VSCI 2019-2022", title = "SSN Predictions (prediction interval 0.90)", subtitle = ("Pearson Correlation = 0.57")) + scale_size_binned(name = "Standard Error")

png(file="figures_nmc_2025/zribbon_pi2.png",width=6,height=3,units="in",res=150)
zribbon_pi2
dev.off()
