...
On 01/24/2025 inserted this code chunk to run status/trend random effects model, ssn_wswq_rand1_st, and compare it to distance model.
```{r st_vs_distance}
...
select(aug_a5predict1ws_vsci, station_id_2, vsci, vsci_pred, .se.fit) %>% arrange(.se.fit) %>% print(n =48)
...
ggplot(aug_a5predict1ws_vsci, aes(x = st_type, y = vsci-pred)) + geom_boxplot()

ggplot(aug_a5predict1ws_vsci, aes(x = st_type, y = .se.fit)) + geom_boxplot()
