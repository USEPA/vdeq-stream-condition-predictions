...
Here we use the distance covariate to test the hypothesis that monitoring sites having upstream waterbodies, such as lakes, reservoirs, and ponds, furthest away would have higher VSCI.
...
# 10 Predictions from Distance WsWQ SSN Model
...
viridis::viridis(7)
tmaptools::palette_explorer() # zoomed to this and checked options Print color values to get color codes for Brewer Categorical Set3 so I could specify fill colors to match tmaps. palette_explorer function is from tmaptools from version 3.1-1. Newer versions of that package use a different function.
