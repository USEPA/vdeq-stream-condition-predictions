...
aug_p_wswq_reml1 <- dplyr::select(aug_z4_wswq_reml1, c(station_id, station_id_2, year, vahusb, do, tn, tothab, l_tn, vscivcpmi, pct_imp_c, pct_imp_w, elev_ws, pct_imp_w_emplog, vsci, .fitted, .std.resid))

# saveRDS(aug_p_wswq_reml1, file = "outputs/aug_p_wswq_reml1.rds")

# st_write(aug_p_wswq_reml1, dsn = file.path(getwd(), "aug_p_wswq_reml1.gpkg"), layer = "aug_p_wswq_reml1", driver = "GPKG", quiet = FALSE, append=FALSE)
...
## 5.2 Torgegram of P_SSN_WSWQ Residuals
```{r torg_wswq_resids}
...
## 5.3 P_SSN_WSWQ Model:  ML Estimation
Because we want to compare model fit and predictions from models having different covariates specifying estimation method as ml is needed.
```{r p_ssn_wswq_ml}
...
# 6.0 Additional Hypotheses for Ws-Wq model
...
a_rssn_wswq_ml1 <- dplyr::select(a_rssn_wswq_ml1, c(station_id, station_id_2, year, vahusb, do, tn, tothab, l_tn, vscivcpmi, pct_imp_c, pct_imp_w, elev_ws, pct_imp_w_emplog, vsci, .fitted, .std.resid))
...
# 8.0 ST_rand Model vs Distance Model
