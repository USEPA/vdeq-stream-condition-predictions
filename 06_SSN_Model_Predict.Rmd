...
# small obs
small_obs = aug_a5obs1ws_vsci %>% select(station_id,st_type,vsci,.fitted,.resid)

p2 = ggplot(small_obs, aes(x = .resid,color = st_type))+
  geom_histogram(aes(label = station_id))
ggplotly(p2,hoverinfo = "text")
# is the .se.fit basically just based on number of samples?
small_dfobsz_n = DFobsz%>% select(station_id,st_type)%>%
  group_by(station_id,st_type)%>%
  summarise(n = n())

# so as far as I can see we actually do not have more samples at the 
