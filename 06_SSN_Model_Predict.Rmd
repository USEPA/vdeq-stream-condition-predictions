...
library(tmap) # for publication quality maps
library(tmaptools)
library(shiny)
...
ggplot(preds3, aes(x = st_type, y = vscivcpmi)) + geom_boxplot() + labs(title = "Preds 2019-2022 (n=48)")

select(preds3, station_id_2, st_type, vscivcpmi) %>% arrange(st_type, station_id_2, vscivcpmi) %>% print(n =Inf)
...
plot(ztg)
plot(ztg, separate = TRUE)
View(ztg$euclid)
View(ztg$flowuncon)  
View(ztg$flowcon) # kind of  low np
...
plot(ztg1)
View(ztg1$euclid)
View(ztg1$flowuncon)  #very low np
View(ztg1$flowcon) #very low np
...
# ST_rand Model vs Distance Model
...
glances(a5ssn_wswq_ml1, ssn_wswq_rand1_st)
...
tidy(np5ssn_wswq_reml1)
summary(np5ssn_wswq_reml1)
varcomp(np5ssn_wswq_reml1)
loocv(np5ssn_wswq_reml1)

glances(a5ssn_wswq_reml1, np5ssn_wswq_reml1)
```
...
select(aug_a5predict1ws_vsci, station_id_2, st_type, vsci, vsci_pred, .se.fit) %>% arrange(.se.fit) %>% print(n =48)

# aug_a5predict1ws_vsci <- aug_a5predict1ws_vsci %>%
#   mutate(
#     st_type = case_when(
#     station_id ==  station_id_2 ~ "status",
#     station_id != station_id_2 ~ "trend"
...
# select(aug_a5predict1ws_vsci, station_id_2, st_type, vsci, vsci_pred, .se.fit) %>% arrange(st_type, .se.fit) %>% print(n =48)

ggplot(aug_a5predict1ws_vsci, aes(x = st_type, y = vsci)) + geom_boxplot() +labs(title = "Observed VSCI at n = 48 Preds Sites")

ggplot(aug_a5predict1ws_vsci, aes(x = st_type, y = vsci_pred)) + geom_boxplot() + labs(title = "Predicted VSCI at n = 48 Preds Sites")

ggplot(aug_a5predict1ws_vsci, aes(x = st_type, y = .se.fit)) + geom_boxplot() + labs(title = "Predicted VSCI at n = 48 Preds Sites")

ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, size = .se.fit, shape = st_type)) + geom_point()

ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, size = .se.fit, shape = st_type)) + geom_point()+ geom_smooth(aes(x = vsci_pred, y = vsci), method = "lm", se = F, color = "blue")

# using linewidth instead of size b/c ggplot geom_smooth waring
ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, linewidth = .se.fit, shape = st_type)) + geom_point()+ geom_smooth(aes(x = vsci_pred, y = vsci), method = "lm", se = F, color = "blue")
...
a5preds2019_2022_v1 <- ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, label = station_id_2)) + geom_point() + geom_abline() + geom_text(aes(label =ifelse(vsci<45, as.character(station_id_2), '')), hjust = -0.1, vjust = 0)+ labs(x = "Predicted VSCI", y = "True VSCI, 2019-2022", title = "a5reml Watershed SSN JU Partitioned & Upstream Waterbody Distance") + xlim(35,80) + ylim(35,80)

# png(file="figures_sfs/a5preds2019_2022_v1.png",width=6,height=3,units="in",res=150)
a5preds2019_2022_v1
# dev.off()

a5preds2019_2022_v2 <- ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, size = .se.fit, shape = st_type, color = st_type)) + geom_point() + labs(x = "Predicted VSCI", y = "True VSCI, 2019-2022", title = "SSN Prediction Sites") + xlim(35,80) + ylim(35,80) + scale_size_binned(name = "Standard Error") + 
scale_shape_manual(name = "Station Type", labels = c("Status", "Trend"))  
  
  guides(shape = guide_legend(title = "Station Type", override.aes = list(size = 6))) + theme(legend.text = element_text(size = 12))

a5preds2019_2022_v2 <- ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, size = .se.fit, shape = st_type, color = st_type)) + geom_point() + labs(x = "Predicted VSCI", y = "True VSCI, 2019-2022", title = "SSN Prediction Sites") + xlim(35,80) + ylim(35,80) + scale_size_binned(name = "Standard Error") + guides(shape = guide_legend(title = "Station Type", override.aes = list(size = 6))) + theme(legend.text = element_text(size = 12))

# This one is closest
a5preds2019_2022_v2 <- ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, size = .se.fit, shape = st_type, color = st_type)) + geom_point() + labs(x = "Predicted VSCI", y = "True VSCI, 2019-2022",  title = "SSN Prediction Sites") + xlim(35,80) + ylim(35,80) + scale_size_binned(name = "Standard Error") + guides(color = guide_legend(override.aes = list(size = 6))) + theme(legend.text = element_text(size = 12))

png(file="figures_nmc_2025/a5preds2019_2022_v2.png",width=6,height=3,units="in",res=150)
a5preds2019_2022_v2
dev.off()
...
ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, size = .se.fit, shape = st_type, color = st_type)) + geom_point() + labs(x = "Predicted VSCI", y = "True VSCI, 2019-2022", title = "SSN Prediction Sites") + xlim(35,80) + ylim(35,80) + scale_size(range = c(4,10), "Standard Error")
...
## NMC Pred VSCI vs True VSCI

```{r nmc_pred_true_vsci_stationtype}
# This one is closest
a5preds2019_2022_v3 <- ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, size = .se.fit, shape = st_type, color = st_type)) + geom_point() + labs(x = "Predicted VSCI", y = "True VSCI, 2019-2022", title = "SSN Prediction Sites", shape = "Station Type", color = "Station Type") + xlim(35,80) + ylim(35,80) + scale_size_binned(name = "Standard Error") + guides(color = guide_legend(override.aes = list(size = 6))) + theme(legend.text = element_text(size = 12))

png(file="figures_nmc_2025/a5preds2019_2022_v3.png",width=6,height=3,units="in",res=150)
a5preds2019_2022_v3
dev.off()

# this plot comes closest
# try ribbon matched to code above
ribbon_pi3 <- ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, shape = st_type, color = st_type)) +
  geom_point(aes(size = .se.fit)) + 
  geom_abline() + 
  geom_ribbon(aes(ymin = vsci_lwr, ymax = vsci_upr), fill = "grey", alpha = .4) +
  geom_line(aes( y = vsci_pred)) + 
  labs(x = "Predicted VSCI", y = "True VSCI 2019-2022", title = "SSN Predictions (prediction interval 0.90)", shape = "Station Type", color = "Station Type", subtitle = ("Pearson Correlation = 0.57")) + scale_size_binned(name = "Standard Error")

png(file="figures_nmc_2025/ribbon_pi3.png",width=6,height=3,units="in",res=150)
ribbon_pi3
dev.off()

+ xlim(35,80) + ylim(35,80) + scale_size_binned(name = "Standard Error") + guides(color = guide_legend(override.aes = list(size = 6))) + theme(legend.text = element_text(size = 12))

ribbon_pi2 <- ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, shape = st_type, label = station_id_2)) +
  geom_point(aes(size = .se.fit)) + 
  geom_abline() + 
  geom_ribbon(aes(ymin = vsci_lwr, ymax = vsci_upr), fill = "grey", alpha = .4) +
  geom_line(aes( y = vsci_pred)) + 
  labs(x = "Predicted VSCI", y = "True VSCI 2019-2022", title = "SSN Predictions (prediction interval 0.90")
...
boxplot_elev_ws <- ggplot(data=DFobs3a, aes(y= elev_ws, x= vahusb)) + geom_boxplot(aes(fill = vahusb), outlier.color = "Red") + geom_jitter() + labs(y = "Watershed Elevation (m)", x = "Subbasins", fill = "Subbasin") + scale_fill_manual(values = c("#8DD3C7", "#FFFFB3", "#BEBADA", "#FB8072", "#80B1D3"))
  
png(file="figures_nmc_2025/boxplot_elev_ws.png",width=6,height=3,units="in",res=150)
boxplot_elev_ws
dev.off()
...
# VAHUSB Polygons
```{r vahusb_poly}
vahusb_poly <- st_read("E:/R_vdeq_nhdplus/vahusb.shp")
# names(vahusb_poly)
# mapview(vahusb_poly)
...
vsci_obs <- mapview(DFobs3a, zcol = "vsci", at=seq(0, 90, 15), cex = "vsci", alpha.regions = .8, legend = TRUE,layer.name = 'Observed Sites VSCI', popup = popupTable(aug_a5predict1ws_vsci, zcol = c("station_id_2", "vsci"))) + mapview(vahusb_poly, zcol="subbasin", burst = TRUE)
vsci_obs

vsci_true <- mapview(aug_a5predict1ws_vsci, zcol = "vsci", at=seq(0, 90, 15), cex = "vsci", alpha.regions = .8, legend = TRUE,layer.name = 'Prediction Sites True VSCI', popup = popupTable(aug_a5predict1ws_vsci, zcol = c("station_id_2", "vsci"))) + mapview(vahusb_poly, zcol="subbasin", burst = TRUE)

#+ mapview(rfbc$edges) + mapview(huc12, zcol = "NAME", burst = TRUE)
vsci_true

sync1 <- sync(vsci_obs, vsci_true, ncol=1)
# mapshot2(sync1, file = sync1)

vsci_pred <- mapview(aug_a5predict1ws_vsci, zcol = "vsci_pred", at=seq(0, 90, 15), cex = "vsci_pred", alpha.regions = .8, legend = TRUE,layer.name = 'Predictands VSCI', popup = popupTable(aug_a5predict1ws_vsci, zcol = c("station_id_2", "vsci_pred")))
#+ mapview(rfbc$edges) + mapview(huc12, zcol = "NAME", burst = TRUE)
vsci_pred

# synced maps1
sync2 <- sync(vsci_true, vsci_pred, ncol =1)

vsci_predse <- mapview(aug_a5predict1ws_vsci, zcol = ".se.fit",  cex = ".se.fit", alpha.regions = .8, legend = TRUE,layer.name = 'Predictands VSCI SE', popup = popupTable(aug_a5predict1ws_vsci, zcol = c("station_id_2", "st_type", "vsci", "vsci_pred", ".se.fit")))

vsci_predse
# synced maps2
sync3 <- sync(vsci_pred, vsci_predse, ncol=1)


```

## NMC tmap: Prediction Maps
This code came from vdeq.RMD under James ProbMon Sites NMC/SFS. I want to modify to show observed vsci vs predictand true vsci, then predictand true vsci vs predictand vsci, and then predictand vsci vs predictand standard error. Best just to make makes separately then align them in Powerpoint. The tmap_arrange function seems to mess up the bubble sizes.
```{r jprobmon}
...
# Observed sites
### trying legend.show = F for polygons
...
j_map_vsci_true <- tm_shape(vahusb_poly) + tm_polygons("subbasin", legend.show = F) +
  tm_shape(aug_a5predict1ws_vsci) + 
  tm_bubbles(
  size = "vsci",
  legend.size.show = FALSE,
  col = "vsci",
...
j_map_vsci_predse <- tm_shape(vahusb_poly) + tm_polygons("subbasin", legend.show = F) +
  ...
  style = "cont",
  palette = "viridis",
  title.col = "Standard Error of Predictand VSCI at Prediction Sites 2019-2022 (n = 48)",
  legend.hist = FALSE,
) + tm_layout(asp=0,
  legend.width = 2, legend.text.size = 1.5, legend.title.size = 3.0,
  frame = FALSE,
...
tmap_save(j_map_vsci_predse, "figures_nmc_2025/j_map_vsci_predse_v1_20250213.jpeg", width = 7, height = 3.5, dpi = 150)



# j_map_obs_true <- tmap_arrange(j_map_vsci_obs, j_map_vsci_true, ncol =1)
# tmap_save(j_map_obs_true, "figures_nmc_2025/j_map_obs_true_v1_20250211.png", width = 6, height = 3, dpi = 150)

# dimensions used that looked good in sfs 2024 powerpoin
# png(file="figures_sfs/esv2.png",width=6,height=3,units="in",res=150)

# RStudio exported map
...
# Preds for ST_rand model reml
Using reml estimate as testing predictions.
```{r st_rand1_hypothesis}

ssn_wswq_rand1_st_reml <- ssn_lm(
  formula = vsci ~ pct_imp_w_emplog + elev_ws + do + vahusb_JL + dist_nearest_up_wb_km + st_type,
  ssn.object = j_ssn3,
  tailup_type = "none",
  taildown_type = "exponential",
...
summary(ssn_wswq_rand1_st_reml)
varcomp(ssn_wswq_rand1_st_reml) # now space varcomp small
loocv(ssn_wswq_rand1_st_reml)

# augment returns an sf object
# note level set to 0.90
aug_st5predict1ws_vsci <- augment(ssn_wswq_rand1_st_reml, newdata = "Preds_2019_2022", se_fit = TRUE, interval = c("prediction"), level = 0.90)
# class(aug_a5predict1ws_vsci)
# names(aug_a5predict1ws_vsci)
aug_st5predict1ws_vsci$vsci_pred <- round(aug_st5predict1ws_vsci$.fitted,1)
aug_st5predict1ws_vsci$vsci_lwr <- round(aug_st5predict1ws_vsci$.lower,1)
aug_st5predict1ws_vsci$vsci_upr <- round(aug_st5predict1ws_vsci$.upper,1)
head(aug_st5predict1ws_vsci$.se.fit)
# Pineiro 2008 put observed on y and predict on x
# label specific points
# https://stackoverflow.com/questions/15624656/label-points-in-geom-point

select(aug_st5predict1ws_vsci, station_id_2, st_type, vsci, vsci_pred, .se.fit) %>% arrange(.se.fit) %>% print(n =48)

cor(aug_st5predict1ws_vsci$vsci_pred,aug_st5predict1ws_vsci$vsci, method = "pearson")

#2019-2022 a5reml correlation with partition factor of ju and distance covariate is 0.57

as.data.frame(aug_st5predict1ws_vsci)|>
  dplyr::select(vsci_pred,vsci)|>
  correlate()
...
ribbon_pi2 <- ggplot(aug_a5predict1ws_vsci, aes(x = vsci_pred, y = vsci, shape = st_type, label = station_id_2)) +
  ...
  geom_ribbon(aes(ymin = vsci_lwr, ymax = vsci_upr), fill = "grey", alpha = .4) +
  geom_line(aes( y = vsci_pred)) + 
  labs(x = "VSCI Predicted 2019-2022", y = "VSCI Observed 2019-2022", title = "SSN Withheld Obs vs Pred a5 reml n=48 interval 0.90, mgm method")
...
small_preds = aug_a5predict1ws_vsci%>% select(station_id,st_type,vsci,vsci_pred,.se.fit,.fitted,.residuals)
...
## loocv
aug_a5obs1ws_vsci$se.fit <- loocv(a5ssn_wswq_reml1,se.fit = T)[[2]]
