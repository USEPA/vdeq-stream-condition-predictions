...
# 1.0 SSN Import & Extract Data Frame
...
# 2.0 Create Distance Matrix
SSN2 creates a distance folder where distance R objects are stored. Look where the SSN object is located in File Explorer. In our case we are looking for the James_071024_pluspreds.ssn folder and within that is the distance folder. The creation of the distance matrix calculates all three distant types and only needs to be done once as long as the geography of points (observation or sites, and prediction points) and flowlines (edges) have not been altered. The function has already been run so it is commented out. Note that this distance matrix only contains distances for obs. When we go to make predictions with an SSN model we need to make sure and run the create distance function and specify that is also to include distance matrix for the preds.
```{r distance_matrix}
...
## 3.4 Variation of Torgegram Plotting
