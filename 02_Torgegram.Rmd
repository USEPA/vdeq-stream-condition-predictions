...
## Outline
For each of the scripts start with a cleared Environment.
...
Notice in the View of ztg$euclid that columns names of dist, gamma, and np correspond to the x-axis, y-axis, and points plotted in the graph
...
Stations are flow-uconnected when they are on different branches of the stream network and share a confluence. The calculation of average gamma and average distance remain the same, but are restricted to those pairs of stations that are flow-unconnected.
...
Stations are flow-connected when they have an upstream to downstream relationship. The calculation of average gamma and average distance remains the same, but are restricted to those pairs of stations that are flow-unconnected. Why might the flow-connected Toregram look so wonky? 
```{r torg_flow_unconnect}
