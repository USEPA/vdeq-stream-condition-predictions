...
Besides the default plot based on the Torgegram object, we can also use ggplot. Back in script 01 the variogram function in the gstat package has a default cutoff of 1/3 the bounding box diagonal, which gave a distance cutoff of ~ 100km. In the SSN2 package, the Torgegram function uses a default cutoff of 1/2 the bounding box diagonal, which gives a Euclidean distance cutoff of ~ 150 km.
